...
| The craft opened, but has fewer parts than the XML | The game dropped parts on load: unreachable from the root part, or an unknown `partType` | `craft_read` in `summary` mode — the `Warnings` section lists disconnected parts |
...
| The engine does not fire | The part is on a different stage than expected, or is not linked to the command pod | The `Stages` section of the summary |
